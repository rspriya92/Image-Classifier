clc
clear
 folder = 'C:\Users\priya\Desktop\9thsem\package\imageclassification\database';
class={'red rose','lilly flower'};
 imagelist = dir('C:\Users\priya\Desktop\9thsem\package\imageclassification\database\*.jpg');
n = length(imagelist);
for i=1:n
    if(i<9)
        target(i)=0;
...
%%%%%%%%%%%%%%%%%%%%%%%%%% knn classifier%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 [filename1,pathname1,filterindex1]=uigetfile('*.jpg');%%Loading the file from the user interface
 queryimage=[pathname1,filename1];
% target=xlsread('C:\Users\priya\Desktop\9thsem\package\imageclassification\image_label.xls')
 n=length(imagelist);
    v=ones(n+1,27);
    q=ones(1,27);
    distance=ones(1,n);
...
classlabel=class{flabel+1};
fprintf('\n Final result of query image given by KNN is %s with %d iterations of learning and with accuracy %f',classlabel,fkval,fmax)
%x - testing size tr=n-testsize
%y - accuracy
%label the point 
figure,
title('variation of test size')
for i=1:(n-1)
    x1(i)=i;
    if(cl(i)==0)
        plot(x1(i),y1(i),'--*r')
